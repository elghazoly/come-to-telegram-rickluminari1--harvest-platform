    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <header style={{ background: 'linear-gradient(90deg, #0a2d6e, #1a4fa8)' }}
        ...
        <div className="flex items-center gap-3">
          ...
          <h1 className="font-bold text-lg">👨‍🏫 تعيين المعلمين للمواد</h1>
        </div>
        <div className="flex gap-2">
          <button onClick={() => setViewMode('by-subject')}
                  className={`px-3 py-1.5 rounded-lg text-sm font-semibold transition-colors ${
                    viewMode === 'by-subject' ? 'bg-white text-blue-700' : 'bg-white/10 text-white hover:bg-white/20'
                  }`}>
            📚 حسب المادة
          </button>
          <button onClick={() => setViewMode('by-teacher')}
                  className={`px-3 py-1.5 rounded-lg text-sm font-semibold transition-colors ${
                    viewMode === 'by-teacher' ? 'bg-white text-blue-700' : 'bg-white/10 text-white hover:bg-white/20'
                  }`}>
            👨‍🏫 حسب المعلم
          </button>
        </div>
